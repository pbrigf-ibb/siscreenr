...
s <-
  build_screen('screenlog_S01.txt', 'layout_S01.txt', rem.col = c(0,2), zero.to.NA = T) %>%
...
n <- s %>% dplyr::group_by(plate, replica, plated) %>% normalize(variables = vs, method = 'medpolish')

z <- n %>% dplyr::group_by(plate_type) %>% siscreenr::zscore(dev = T, ref = well_type == 'sample', var = vn)
