...
#'
#' @details
#' The function first checks the data files present in the data directory against the plates logged in the screen log.
#' Data files must contain full plate names as given in the screen log. Suffixes are allowed but must be separated by \code{_}.
#' Unexpected and missing plates are reported.
#'
#' Once the plate list has been compiled, the layout file is loaded and all present data files that correspond to logged plates
#' are read and wrapped into a single data frame.
#'
...
#'              0 is interpreted as the last column; defaults to c(0,2)
#'
...
#' It must not contain underscores, all other (legal) characters are allowed. Anything rith of an underscore will be discarded.
#'
#' @section Layout file:
...
#'
#' The file should also specify a well's coordinates on a plate, either as \code{position}, e.g. "E02", or \code{row} and \code{column}.
#' By default ScanR sorts and numbers wells by row, in contrast to the default filling of matrices in R.
#' Thus, \code{\link{normalize}} sorts the data frame by column and row before running the median polish
#' and if these columns are absent it throws an error.
#'
#' @section Dropping columns:
#' Columns are removed upon request. This option exists for two reasons.
#' First, ScanR always adds an empty column at the end called "X". Second, the second column in every exported file,
#' called "Description", contains the names of groups of wells. Since we only ever export data by well and not by group,
...
build_screen <- function(logfile, layout, datadir = './data/', rem.col, zero.to.NA = FALSE, verbose = FALSE) {
  # check arguments
  ...
  if (missing(rem.col)) {
    rem.col <- c(0,2)
  } else if (!is.numeric(rem.col) & !is.character(rem.col)) {
  ...
  if (!is.null(rem.col)) {
    ...
    if (is.character(rem.col)) {
      if (!all(rem.col %in% colnames(scr))) {
        nec <- setdiff(rem.col, colnames(scr))
        warning('request to remove non-existing column(s): ', nec,' was ignored', call. = FALSE, immediate. = TRUE)
        rem.col <- intersect(rem.col, colnames(scr))
      }
      rem <- rem.col %>% paste0('-', .)
    }
    else {
      ...
      rem <- cols[rem.col] %>% paste0('-', .)
    }
    scr <- dplyr::select_(scr, .dots = rem)
  }
